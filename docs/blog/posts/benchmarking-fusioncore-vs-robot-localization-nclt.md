...
**Adaptive noise:** robot_localization uses fixed noise values set in a YAML config. Every GPS fix gets the same weight regardless of HDOP, satellite count, or fix quality. FusionCore scales GPS noise by HDOP from the message, applies a chi-squared Mahalanobis gate to reject statistical outliers, and also adapts its noise covariance estimate from the live innovation sequence using a sliding window. Bad GPS in an urban canyon gets down-weighted automatically. The NCLT campus has a lot of tree cover and building multipath: that difference matters.
